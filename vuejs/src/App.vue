    ...
    <div class="">
      <div class="px-6 pb-12 pt-12 lg:px-8 border-b border-gray-500">
        <h2 class="text-3xl font-bold tracking-tight text-white">
          <a class="link" href="https://unified.to" target="_blank">Unified.to</a> File Manager
          Component
        </h2>
        <p class="normal-text">
          This sample code shows you how to interact with the
          <a class="link" href="https://docs.unified.to/storage/overview" target="_blank">
            Unified.to File Storage API
          </a>
          and provides a file manager component that you can use in your own application.
        </p>
        <p class="normal-text">
      ...
      <div class="px-6 pt-12 lg:px-8">
        <div v-if="connection_id">
          <FileManager :connection_id="connection_id" :token="token" />
        ...
        <div class="lg:mr-0 lg:max-w-lg" v-else-if="integrations.length === 0">
          <p class="normal-text">
            Please
            <a class="link" href="https://app.unified.to/integrations?tab=storage" target="_blank">
              Activate Integrations
            </a>
            in the file storage category first
          </p>
        </div>

        <div v-else-if="!connection_id">
          <UnifiedDirectory :workspace_id="workspace_id" :categories="['storage']" />
...
export default {
  ...
  methods: {
    ...
    async get_integrations() {
      ...
      user.environment = user.environment || 'Production'

      this.integrations = await this.get(
        `unified/integration/workspace/${this.workspace_id}?env=${encodeURIComponent(user.environment)}&categories=storage`
      )
    }
  },
  mounted() {
